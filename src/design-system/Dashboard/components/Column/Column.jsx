...
export const Column = ({ column }) => {
  ...
  return (
    <div data-testid='column' className='border-solid border-gray-400 border-2 rounded-lg p-6 m-4 grid' style={{ gridColumn: `span ${ screenSize.width >= 1280 && column ? column.size : 12 }` }}>
      <h2 data-testid={`column-heading-${ column && column.heading }`} className='font-bold text-xl pb-5 font-heading flex items-center gap-2 mb-auto'><img src={column && column.icon} style={{ width: '25px', height: '25px' }}></img>{column && column.heading}</h2>
